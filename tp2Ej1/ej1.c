...
int ej1() {
	...
	while(1) {
		...
		switch (swn) {
			// Show segs
			case (1<<0) : showingMode = SHOW_SEGS; running = 1; _delay_ms(500); break;
			// Show mins
			case (1<<1) : showingMode = SHOW_MINS; running = 1; _delay_ms(500); break;
			// Show hours
			case (1<<2) : showingMode = SHOW_HOURS; running = 1; _delay_ms(500); break;
			// Config segs
			case (1<<3) : showingMode = SHOW_SEGS; running = 0; _delay_ms(500); break;
			// Config mins
			case (1<<4) : showingMode = SHOW_MINS; running = 0; _delay_ms(500); break;
			// Config hours
			case (1<<5) : showingMode = SHOW_HOURS; running = 0; _delay_ms(500); break;
			// Config +
			case (1<<6) : swsPlus(); _delay_ms(500); break;
			// Config -
			case (1<<7) : swsMinus(); _delay_ms(500); break;
